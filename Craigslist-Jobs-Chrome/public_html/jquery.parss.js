// Modified for the Philly Jobs extension by J.Mckibbin (http://joshmckibbin.com)
		  ...
		  a.fn.PaRSS=function(m,c,b,l){
			  ...
			  function f(){
				  var n=new google.feeds.Feed(m);if(g.item_count){n.setNumEntries(g.item_count)}n.load(function(o){if(!o.error){d(o.feed.entries)}})}function d(n){
					...
					list_item="<span class='parss-title'><a href='#' class='toggle' onclick='desc(this); return false;'>"+q.title+"</a></span>";
					
					if(g.date_format&&g.date_format.length>0){
						list_item+="<span class='parss-date'>"+k(q.publishedDate,g.date_format)+"</span>"
					}
					
					switch(g.show_descriptions){
						case"image":
						list_item+="<span class='parss-image'>"+h(q.content)+"</span><div class='parss-description'>"+q.contentSnippet+" <a href='"+q.link+"' target='_blank' class='apply'>VIEW POST / APPLY</a></div>";
						break;
						case"content":
						repl = q.content;
						list_item+="<div class='parss-description'>"+linkify(repl)+" <a href='"+q.link+"' target='_blank' class='apply'>VIEW POST / APPLY</a></div>";
						break;
						case true:
						case"true":
						list_item+="<div class='parss-description'>"+q.contentSnippet+" <a href='"+q.link+"' target='_blank' class='apply'>VIEW POST / APPLY</a></div>";
						break;
						default:
						break;
					}
					o+="<div class='parss-entry'>"+list_item+"</div>"
					});
