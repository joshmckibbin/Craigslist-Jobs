function desc(elem){
	totoggle = $(elem).parent().siblings(".parss-description");
	toanchor = $(elem).parent(".parss-title");
	totoggle.slideToggle(400,function(){
		if($(".parss-description").is(":visible")){
			$(".parss-description").not(totoggle).slideUp(400, function(){
				//$('body').scrollTop(toanchor.position().top);
				//toanchor.animate({scrollTop:0},2000,'ease');
				toanchor.children("a").css("color","red");
			});
...
$(document).ready(function(){
	...
	if(!city){
		...
		$("#main-content").PaRSS(preurl+type+"/index.rss", qty, "M jS g:i a", "content");
